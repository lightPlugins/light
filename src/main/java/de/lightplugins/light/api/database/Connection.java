package de.lightplugins.light.api.database;
...
public class Connection {
    ...
    public Connection(Light light) {
        this.light = light;
    }

    public void connectMariaDB() {

        FileConfiguration config = Light.database.getConfig();
        ...
        Light.api.getDebugPrinting().print("Connected to MariaDB");
    }
    ...
    public void connectSQLite() {
        ...
        if (!sqlFile.exists()) {
            try {
                if(!sqlFile.createNewFile()) {
                    ...
                    Light.api.getDebugPrinting().print("&cCannot create &4" + database);
                    return;
                }
            } catch (IOException e) {
            ...
            Light.api.getDebugPrinting().print("SQLite file already exists -> skipping");
        }
        ...
        Light.api.getDebugPrinting().print("Connected to SQLite");
    }
